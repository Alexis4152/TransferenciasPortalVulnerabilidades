		    ...
		    items: [ 
		        { 
		            ...
		            'click': function(){
		            	...
		            	var redirect = 'TemplateAction.action?parameter=detalleTransferenciaCompensacion&idTransferencia=<c:out value="${transferencia.idtransferencia}"/>&excel=excel'; 
	                 //   alert(redirect);
...
//
  ...
  <c:if test="${estatus == 7 && documentosCompletos}">
 		new  Ext.Button({
	      	   id:'botonAplicaExt',
	      	   text:'Programar compensacion',
	      	   width:'120',	      	                
	      	   applyTo:'botonAplicaCompensacion',
	      	   ...
	      	   listeners:{
		            'click': function(){
		                ...
		                ajaxDivUpdater('TemplateAction.action','parameter=detalleTransferenciaExecuteCompensacion&aplica=${transferencia.idtransferencia}_${tipoTrans}_${transferencia.idsCompensacionString}&pendiente=${pendiente}','dsplay_none'); 
		            }
		    	}
	      });
	...
	<c:if test="${estatus == 19}">
	
		new  Ext.Button({
      	   id:'botonRegresarEstatusExt',
      	   text:'Regresar a aplicada',
      	   width:'120',	      	                
      	   applyTo:'botonRegresarEstatus',
      	   <c:if test="${sessionScope.empleado.idPefril==3 || sessionScope.empleado.idPefril==5  || sessionScope.empleado.idPefril==4}">
						 disabled:true,
						 hidden:true,
		   </c:if>	 
      	   listeners:{
	            'click': function(){
	           		mascara(true);
	            	//alert(Ext.Ajax.serializeForm('listaAplicaForm'))
	                ajaxDivUpdater('TemplateAction.action','parameter=detalleTransferenciaExecuteCompensacionError&aplica=${transferencia.idtransferencia}_${tipoTrans}&pendiente=${pendiente}','dsplay_none'); 
	            }
	    	}
      });
		...
		<table width="100%" id="the-table-h">
			...
			<thead>
				<tr style="background: #eeeeee;">
					...
					<th>Numero de documento compensacion</th>
				</tr>
			</thead>
			...
			<tbody>
				<c:forEach var="pagos" items="${listaPagos}" varStatus="a">
					<tr>
						...
						<td>${pagos.numeroDocumentoCompensacion}</td>
				...
				<tr>
					<td>
						<div>
							...
							<div align="center" id="botonAplicaCompensacion"></div>

							<div align="center" id="botonAutorizaDetalle"></div>

							<div align="center" id="botonSoltarDetalle"></div>
							
							<div align="center" id="botonRegresarEstatus"></div>
