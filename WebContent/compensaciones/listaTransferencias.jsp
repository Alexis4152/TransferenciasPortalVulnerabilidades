	      	   ...
	      	   listeners:{
		            'click': function(){
		              ...
		              try{
		                ...
		                ajaxDivUpdater('TemplateAction.action','parameter=detalleTransferenciaExecuteCompensacion&pendiente=true&lista=lista&'+Ext.Ajax.serializeForm('formadeslgosa'),'listaTransferenciaPendiente'); 
		            }
		    	}
	      });  
	      </c:if>       
...
<table  width="100%" id="the-table-h"  >
        ...
        <tbody>
      	 <c:forEach var="trans" items="${listaTrasferencia}">
	            <tr>
	                <c:if test="${titulo=='Pendientes'}">
	                <td><input name="aplica" id="${trans.idtransferencia}_${trans.tipoPagos}" value="${trans.idtransferencia}_${trans.tipoPagos}_${trans.idsCompensacionString}" type="checkbox"/>  </td>
	                </c:if>
	                ...
	                <td>${trans.cliente}</td>
					<td>${trans.alias}</td>
	                <td><a href="javascript:addTabDetalle('TemplateAction.action','parameter=detalleTransferenciaCompensacion&idTransferencia=${trans.idtransferencia}&tipoTrans=${trans.tipoPagos}&pendiente=${pendiente}')">Ver</a>  </td>
